...
public class LoginController {
  ...
  //  @PostMapping("/login")
    public String login(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletResponse httpServletResponse){
        log.info("id : {}, password: {}",loginForm.getLoginId(),loginForm.getPassword());
        ...
        if(loginMember == null){
            bindingResult.reject("loginFail","아이디 또는 비밀번호 불일치");
            return "login/loginForm";
        }
 ...
 //   @PostMapping("/login")
    public String loginV3(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult, HttpServletResponse httpServletResponse, HttpServletRequest req){
        log.info("id : {}, password: {}",loginForm.getLoginId(),loginForm.getPassword());
        if(bindingResult.hasErrors()){
            return "login/loginForm";
        }
    ...
    public String loginV4(@Valid @ModelAttribute LoginForm loginForm, BindingResult bindingResult,
                          HttpServletResponse httpServletResponse, HttpServletRequest req,
